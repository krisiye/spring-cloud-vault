 ...
 *
 */
public class AwsBackendSTSFederationTokenMetadata implements LeasingSecretBackendMetadata {
	...
	AwsBackendSTSFederationTokenMetadata(VaultAwsProperties properties, PropertyTransformer transformer,
		...
		container.addLeaseListener(leaseEvent -> {
			...
			if (leaseEvent.getSource() == secret && leaseEvent instanceof SecretLeaseCreatedEvent) {
				if (this.log.isDebugEnabled()) {
					this.log.debug("Publishing a RebindAwsStsFederationTokenEvent");
				}
				this.eventPublisher.publishEvent(new RebindAwsStsFederationTokenEvent(this));
			}
		});

	}

	public static class RebindAwsStsFederationTokenEvent extends ApplicationEvent {

		RebindAwsStsFederationTokenEvent(Object source) {
			super(source);
		}
