...
/**
 * Bootstrap configuration providing support for the AWS secret backend defaults to
 * iam_user.
 *
 * @author Mark Paluch
 * @author Kris Iyer
...
// @ConditionalOnProperty(name = "spring.cloud.vault.aws.credentialType", havingValue =
// "iam_user", matchIfMissing = true)
public class VaultConfigAwsIAMUserBootstrapConfiguration {
	...
	public static class AwsSecretBackendMetadataFactory implements SecretBackendMetadataFactory<VaultAwsProperties> {
		 ...
		 */
		SecretBackendMetadata forAws(final VaultAwsProperties properties) {
			...
			return new AwsBackendIAMuserMetadata(properties, transformer);
		}
