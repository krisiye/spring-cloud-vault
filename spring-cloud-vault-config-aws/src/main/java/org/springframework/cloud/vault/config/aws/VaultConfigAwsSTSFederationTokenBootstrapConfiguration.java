...
/**
 * Bootstrap configuration providing support for the AWS secret backend and STS federation
 * Token.
 *
 * @author Kris Iyer
 */
...
@ConditionalOnProperty(name = "spring.cloud.vault.aws.credentialType", havingValue = "federation_token")
public class VaultConfigAwsSTSFederationTokenBootstrapConfiguration {
	...
	public static class AwsSecretBackendMetadataFactory implements SecretBackendMetadataFactory<VaultAwsProperties> {
		...
		LeasingSecretBackendMetadata forAws(final VaultAwsProperties properties) {
			...
			Assert.isTrue(properties.getCredentialType().equalsIgnoreCase(AWSCredentialType.federation_token.name()),
			...
			return new AwsBackendSTSFederationTokenMetadata(properties, transformer, this.publisher);
		}
